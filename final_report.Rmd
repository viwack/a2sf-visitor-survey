...
combined_zip_count_all %>%
  ...
  ggplot(aes(reorder(county, -count), frac, fill = county)) + geom_bar(stat = "identity") + theme(axis.text.x = element_text(angle = 30, vjust = 0.7, hjust = 0.5)) + geom_text(aes(label = frac), vjust = -0.1, position = position_dodge(0.9), size = 2) + theme(legend.position = "none") + labs(y = "Proportion of Visitors", x = "County", title = "Visitors by County - All")
```
...
clean_all %>%
  ...
  summarise(count = n()) %>%
  mutate(frac = round(count/sum(count),2)) %>%
  ggplot(aes(x = `What is your age`, y = count, fill = source)) + geom_bar(stat = "identity", position = "dodge") + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + labs(x = "Age Group", y = "Count", title = "Respondents by Age and Survey Type") + geom_text(aes(label = frac), vjust = -0.1, position = position_dodge(0.9), size = 2)
```
...
*All unique race/ethnicity identities*
...
clean_all %>%
  ...
  labs(x = "Income Bracket", y = "# of Respondents", title = "Visitors by Gross Household Income") + theme(axis.text.x = element_text(angle = 26, hjust = 1)) +  scale_fill_brewer("PuRd") + theme(legend.position = "none") + geom_text(aes(label = fraction), vjust = -0.1, position = position_dodge(0.9), size = 2)
...
clean_all %>%
  filter(!is.na(`Respondent ID`)) %>%
  group_by(`What was your gross household income during the last year`,source) %>%
  summarize(count = n()) %>%
  mutate(fraction = round(count/sum(count),2)) %>%
  filter(`What was your gross household income during the last year` != "Decline to answer") %>%
  ggplot(aes(x = `What was your gross household income during the last year`, y = count, fill = source)) + geom_bar(stat = "identity", position = "dodge") + 
  labs(x = "Income Bracket", y = "# of Respondents", title = "Visitors by Gross Household Income") + theme(axis.text.x = element_text(angle = 26, hjust = 1)) + geom_text(aes(label = fraction), vjust = -0.1, position = position_dodge(0.9), size = 2)
```
